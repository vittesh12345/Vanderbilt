...
console.log("\n[4b] Campus: researched opportunities and their deadlines");
await page.locator(".tab", { hasText: "Campus" }).click();
await page.waitForTimeout(200);
let camp = await page.locator("#root").innerText();
ok(camp.includes("Closing soon"), "closing-soon section present");
ok(camp.includes("Wond'ry IMPACT"), "the IMPACT deadline is surfaced");
ok(camp.includes("Vanderbilt Investment Banking Club"), "the VIBC deadline is surfaced");
ok(/researched 2026-08-28/.test(camp), "says when the data was gathered");
ok(camp.includes("read cleanly") || camp.includes("check this") || /READ CLEANLY|CHECK THIS/.test(camp), "confidence shown per entry");
const clubCount = await page.locator("#root .card").count();
ok(clubCount >= 20, "clubs listed, got " + clubCount);
await page.locator("input.in[placeholder*='Filter']").fill("consulting");
await page.waitForTimeout(200);
const filtered = await page.locator("#root .card").count();
ok(filtered > 0 && filtered < clubCount, "filter narrows the list: " + filtered + " of " + clubCount);
await page.locator("input.in[placeholder*='Filter']").fill("");
await page.waitForTimeout(150);
await page.locator("button", { hasText: "Labs" }).first().click();
await page.waitForTimeout(200);
const labs = await page.locator("#root").innerText();
ok(/takes undergrads/i.test(labs), "labs show undergraduate access");
ok(/Next step:/.test(labs), "labs carry a concrete next action");

console.log("\n[4c] tracking a deadline puts it in the ranked work");
await page.locator("button", { hasText: "Wond'ry" }).first().click();
await page.waitForTimeout(150);
await page.getByRole("button", { name: "Track this" }).first().click();
await page.waitForTimeout(250);
await page.locator(".tab", { hasText: "Now" }).click();
await page.waitForTimeout(200);
const nowAfter = await page.locator("#root").innerText();
ok(/IMPACT/.test(nowAfter), "the tracked application now appears in Do this next");
ok(/no make-up|cannot make/i.test(nowAfter), "its reason says a missed deadline cannot be recovered");
ok(!/IMPACT.*not tracking it/s.test(nowAfter), "the untracked nag stops once it is tracked");
...
ok(savedState.items.length === 3, "published state kept 3 work items incl. the tracked application, got " + savedState.items.length);
ok(savedState.items.some(i => i.kind === "APPLICATION" && i.ref), "the tracked opportunity persisted with its source reference");
